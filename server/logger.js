...
module.exports = function(options) {
  ...
  return async function logger(ctx, next) {
    ...
    const start = ctx[Symbol.for('request-received.startTime')]
      ...
      : Date.now();
    print(
      `  ${chalk.gray('<--')} ${chalk.bold('%s')} ${chalk.gray('%s')} ${chalk.yellow('%s')}`,
      ctx.method,
      ctx.originalUrl,
      ctx.ip
    );
...
function log(print, ctx, start, length_, err, event) {
  ...
  const length = [204, 205, 304].includes(status) ? '' : length_ == null ? '-' : bytes(length_).toLowerCase();

  const upstream = err ? chalk.red('xxx') : event === 'close' ? chalk.yellow('-x-') : chalk.gray('-->');

  print(
    `  ${upstream} ${chalk.bold('%s')} ${chalk.gray('%s')} ${chalk[color]('%s')} ${chalk.gray('%s')} ${chalk.gray(
      '%s'
    )}`,
    ctx.method,
    ctx.originalUrl,
